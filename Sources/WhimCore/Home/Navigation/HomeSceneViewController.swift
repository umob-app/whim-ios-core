...
/// Basic building block for representing any value in the context of possible home view controller variation.
public enum HomeSceneViewControllerValue<T> {
    case fullscreen(T)
    case multipart(top: T, bottom: T)
}

extension HomeSceneViewControllerValue: Equatable where T: Equatable {}

public extension HomeSceneViewControllerValue {
    var fullscreen: T? {
...
/// Represents available scenarios to render view controllers within home flow.
public typealias HomeSceneViewController = HomeSceneViewControllerValue<UIViewController>

public extension HomeSceneViewController {
...
public extension HomeSceneViewControllerValue {
    /// Returns a home view controller containing the results of mapping the given closure over the original value.
    ...
    @discardableResult
    func map<U>(fullscreen: (T) -> U, multipart: (_ top: T, _ bottom: T) -> (top: U, bottom: U)) -> HomeSceneViewControllerValue<U> {
        switch self {
    ...
    @discardableResult
    func map<U>(_ transform: (T) -> U) -> HomeSceneViewControllerValue<U> {
...
public extension HomeSceneViewControllerValue where T: AnyObject {
    /// Sets a property of the value by the given `keyPath` with the value wrapped into other home view controller.
    /// Both, current and new home view controllers should be of the same case, otherwise nothing will happen.
    func update<U>(keyPath: WritableKeyPath<T, U>, with value: HomeSceneViewControllerValue<U>) {
        switch (self, value) {
