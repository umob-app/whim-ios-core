...
/// Scene that represents ordered navigation stack with view controllers within home flow.
///
...
open class HomeSceneNavigationStack: HomeScene {
    /// Fullscreen white scene.
    private static let placeholder: HomeSingleScene = {
        let viewController = UIViewController()
        viewController.view.backgroundColor = .white
        return HomeSingleScene(fullscreen: viewController)
    }()

    /// Ordered scenes stack.
    public final private(set) var scenes: [HomeScene] = []

    /// A parent-child relationship. Publicly available for read-only access.
    public final let relationship = HomeSceneRelationship()
    ...
    private var root: HomeScene {
        return scenes.first ?? Self.placeholder
    }
    private var current: HomeScene {
        return scenes.last ?? Self.placeholder
    }

    public init(_ scenes: [HomeScene]) {
        for scene in scenes {
            add(scene: scene)
        }
    }

    public convenience init(_ head: HomeScene, _ tail: HomeScene...) {
        self.init([head] + tail)
    }
    ...
    public final func push(scene: HomeScene, animating: HomeSceneAnimatedTransitioning? = HomeSceneAnimatedTransitions.Push()) {
    ...
    public final func push(scenes: [HomeScene], animating: HomeSceneAnimatedTransitioning? = HomeSceneAnimatedTransitions.Push()) {
    ...
    @discardableResult
    private func add(scene: HomeScene) -> Bool {
    ...
    private func canAdd(scene: HomeScene) -> Bool {
    ...
    public final func pop(
        animating: HomeSceneAnimatedTransitioning? = HomeSceneAnimatedTransitions.Pop()
    ) -> HomeScene? {
    ...
    public final func pop(
        ...
        andSwapLastWith scene: HomeScene? = nil,
        animating: HomeSceneAnimatedTransitioning? = HomeSceneAnimatedTransitions.Pop()
    ) -> (popped: [HomeScene], swapped: Bool) {
    ...
    public final func swapAll(
        with scene: HomeScene,
        animating: HomeSceneAnimatedTransitioning? = HomeSceneAnimatedTransitions.None()
    ) -> Bool {
    ...
    /// - Note: Do not call this method directly. It's an implementation of `HomeSceneResponder` protocol.
