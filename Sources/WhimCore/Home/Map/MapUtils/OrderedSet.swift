// original: https://github.com/apple/swift-package-manager/blob/master/swift-tools-support-core/Sources/TSCBasic/OrderedSet.swift
...
public struct OrderedSet<E: Hashable>: Equatable, Collection {
    ...
    /// Insert an element in the ordered set unconditionally.
    ///
    /// If it already contains the element, it will be updated.
    /// If it doesn't contain the element, it will be appended.
    public mutating func update(with newElement: Element) {
        if set.update(with: newElement) == nil {
            array.append(newElement)
        } else {
            let idx = array.firstIndex(of: newElement)!
