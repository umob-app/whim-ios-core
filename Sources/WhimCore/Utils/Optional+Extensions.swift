extension WhimCore {
    public static func zip<A, B>(_ a: A?, _ b: B?) -> (A, B)? {
        return a.flatMap { a in b.map { b in (a, b) } }
    }

    public static func zip<A, B, C>(_ a: A?, _ b: B?, _ c: C?) -> (A, B, C)? {
        return zip(a, b).flatMap { a, b in c.map { c in (a, b, c) } }
    }

    public static func zip<A, B, C, D>(_ a: A?, _ b: B?, _ c: C?, _ d: D?) -> (A, B, C, D)? {
