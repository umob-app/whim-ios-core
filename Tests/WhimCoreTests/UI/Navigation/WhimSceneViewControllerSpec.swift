...
final class WhimSceneViewControllerSpec: QuickSpec {
    override func spec() {
        describe("WhimSceneViewControllerValue") {
            describe("when mapping") {
                context("each case separately") {
                    it("should return transformed value for the same option") {
                        let sut = WhimSceneViewControllerValue.multipart(top: 42, bottom: 24).map(
                            fullscreen: { "\($0)" },
                ...
                context("with a different view controller value") {
                    context("of a different option") {
                        it("should not do anything") {
                            let obj = TestObject()
                            WhimSceneViewControllerValue.fullscreen(obj).update(
                                keyPath: \.value,
                    ...
                    context("of the same option") {
                        it("it should update its corresponding values") {
                            ...
                            WhimSceneViewControllerValue.multipart(top: top, bottom: bottom).update(
                                keyPath: \.value,
                                with: .multipart(top: 42, bottom: 24)
                            )
